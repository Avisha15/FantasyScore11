...
export const getTransactionsDeposit = () => async (dispatch: Dispatch<any>) => {
  try {
    dispatch(setLoading(true));
    const response = await appOperation.customer.deposit_tran();
...
export const getTransactionsContest = () => async (dispatch: Dispatch<any>) => {
  try {
    dispatch(setLoading(true));
    const response = await appOperation.customer.contest_tran();

    if (response?.success) {
      dispatch(setTransactionsContest(response?.data));
    }
  } catch (e) {
    logError(e);
  } finally {
...
export const getTransactionsWithdrawals =
  () => async (dispatch: Dispatch<any>) => {
    try {
      dispatch(setLoading(true));
      const response = await appOperation.customer.withdrawal_tran();

      if (response?.success) {
        dispatch(setTransactionsWithdrawals(response?.data));
      }
    } catch (e) {
      logError(e);
    } finally {
